...
public:
	...
	void CalcCircleParameters(TVector<TPipelineElement> &pipe)//TODO пока что здесь все дуги разбиваются на мелкие линейные отрезки
	{
	...
	void MakePipe(TVector<TPipelineElement> &pipe,TATPTokenizer &atp_tokens)
	{
		...
		for(int i=0;i<atp_tokens.GetTokensCount();i++)
		{
			try{
				...
				{
					if(atp_tokens[i].params_count==3)//TODO при смене инструмента у нас сбрасываются обороты шпинделя
					{
	...
	void GetCode(TVector<TToolMovementElement<T>> &pipe,string &result_code,const char* ext_header,const char* prog_id)//только для standalone постов, далее убрать
	{

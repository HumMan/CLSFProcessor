...
void compiler::PrepareCommandInsertion(CLSFNodes &nodes,list<TMovement>* &movements,list<TMovementContour>* &contours,boost::ptr_vector<TMovementCommand>* &commands)const
{
	movements=&nodes.curr_tool_path->movements;

	bool curr_contour_params_change=false;
	
	if(movements->size()==0||(movements->back().type!=nodes.curr_move_type))
	{
		if(movements->size()==0)
			curr_contour_params_change=true;
		else
			curr_contour_params_change=(movements->back().contours.back()!=nodes.curr_contour_params);

		movements->push_back(TMovement());
		movements->back().type=nodes.curr_move_type;
	}

	contours=&movements->back().contours;
	if(contours->size()==0||(contours->back()!=nodes.curr_contour_params))
	{
		if(contours->size()!=0)
			curr_contour_params_change=contours->back()!=nodes.curr_contour_params;
		contours->push_back(TMovementContour());
		*(TMovementContourParams*)(&contours->back())=nodes.curr_contour_params;
		contours->back().contour_params_change=curr_contour_params_change;
	}

	commands=&contours->back().commands;
}
...
void TProgram::GetGCode(std::string& program)
{
	...
	for(path=tool_paths.begin();path!=tool_paths.end();path++)
	{
		...
		for(movement=path->movements.begin();movement!=path->movements.end();movement++)
		{
			...
			//for(int c=0;c<movement->contours.size();c++)
			{
